...
if [ $stage -le 8 ]; then
  ...
  steps/nnet3/lstm/train.sh --stage $train_stage \
    ...
    data/train_hires data/lang_bd exp/tri3_ali_bd $dir  || exit 1;
fi
...
if [ $stage -le 9 ]; then
  ...
  for t in eval dev; do
      (
	  num_jobs=`cat data/${t}_hires/utt2spk|cut -d' ' -f2|sort -u|wc -l`
	  steps/nnet3/decode.sh \
	      ...
	      $graph_dir data/${t}_hires $dir/decode_tg_bd_${t} || exit 1;
	  steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" \
          data/lang_{tg,fg}_bd data/${t}_hires \
          $dir/decode_{tg,fg}_bd_$t || exit 1;
      ) &
  done
