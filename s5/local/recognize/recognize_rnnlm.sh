...
LM=3gsmall    # Old LM
decodedir=${datadir}_segm_hires/decode_$LM
rescoredir=${decodedir}_rnnlm
...
if [ $stage -le 6 ]; then
    ...
    rnnlm/lmrescore_pruned.sh \
      ...
      --skip-scoring true \
      data/lang_$LM $rnnlmdir \
      ${datadir}_segm_hires ${decodedir} \
      ${rescoredir} || exit 1;
...
if [ $stage -le 7 ]; then
    ...
    lattice-best-path \
        --lm-scale=$lmwt \
        --word-symbol-table=data/lang_$LM/words.txt \
...
if [ $score = true ] ; then
	...
	--cmd "$train_cmd" $speechname data/lang_$LM ${datadir}/.. || exit 1;
fi

rm -r ${datadir} ${datadir}_segm
#rm -r ${datadir}_segm_hires
