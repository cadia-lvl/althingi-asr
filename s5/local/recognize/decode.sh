...
#           2018       Reykjavik University (Inga Run Helgadottir): Recognition of new speeches using existing models
# Apache 2.0.
...
if [ ! -z "$model_dir" ]; then
  model=$model_dir/$iter.mdl || exit 1
fi

utils/lang/check_phones_compatible.sh {$model_dir,$graphdir}/phones.txt || exit 1
...
sdata=$data/split$nj;
cmvn_opts=`cat $model_dir/cmvn_opts` || exit 1;
thread_string=
...
if [ -f $model_dir/frame_subsampling_factor ]; then
  ...
  frame_subsampling_opt="--frame-subsampling-factor=$(cat $model_dir/frame_subsampling_factor)"
fi
